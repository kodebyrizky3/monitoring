(function () {
  ...
  const AVATAR_PLACEHOLDER = (window.APP && window.APP.avatarPlaceholder) ? window.APP.avatarPlaceholder : '/assets/img/avatar-placeholder.png';

  function debounce(fn, ms) { let t; return (...a) => { clearTimeout(t); t = setTimeout(() => fn(...a), ms); }; }
  function escapeHtml(s) { return (s || '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function pageRange(current, total, width) { const half = Math.floor(width / 2); let start = Math.max(1, current - half); let end = Math.min(total, start + width - 1); start = Math.max(1, end - width + 1); const arr = []; for (let i = start; i <= end; i++) arr.push(i); return arr; }
  ...
  function setInfoCount(rowsLen) {
    ...
    infoEl.textContent = `Menampilkan ${start}–${end}` + (state.q ? ' · Urut: relevansi' : '');
  }
  ...
  async function fetchList() {
    ...
    const u = new URL(window.APP?.pegawaiSearch || '/admin/pegawai/search', window.location.origin);
      ...
      tbody.innerHTML = rows.length ? rows.map((r, i) => {
        ...
        const aksiActive = `
          <div class="btn-group btn-group-sm">
            ...
            <button class="btn btn-outline-danger btn-delete" data-id="${r.id}" data-url="${r.delete_url}" data-name="${escapeHtml(r.nama)}"><i class="bi bi-archive"></i></button>
          </div>`;
        const aksiArchived = `
          <div class="btn-group btn-group-sm">
        ...
        const aksi = (state.filter === 'archived') ? aksiArchived : aksiActive;
  ...
  function renderPager() {
    ...
    const btn = (label, page, disabled=false, active=false) => `<li class="page-item ${disabled?'disabled':''} ${active?'active':''}"><a class="page-link" href="#" data-page="${page}">${label}</a></li>`;
  ...
  function linkInstagram(u) {
    ...
    return `<a href="${href}" target="_blank" rel="noopener">@${escapeHtml(handle)}</a>`;
  }
  ...
  function renderDetail(d) {
    ...
    if (kodeEl) kodeEl.textContent = d.kode_pegawai ? `Kode: ${d.kode_pegawai}` : 'Kode: —';
    if (bidEl)  bidEl.textContent  = d.bidang_nama ? `Bidang: ${d.bidang_nama}` : 'Bidang: —';
  ...
  function bindRowActions() {
    document.querySelectorAll('.btn-detail').forEach(btn => btn.onclick = async () => {
      ...
      try {
        ...
        if (window.Swal) Swal.fire({icon:'error',title:'Gagal',text:e.message||'Terjadi kesalahan'}); else alert(e.message||'Terjadi kesalahan');
      }
    });
    ...
    document.querySelectorAll('.btn-edit').forEach(btn => btn.onclick = async () => {
      ...
      clearErrors();
      try {
        const url = (window.APP?.pegawai || '/admin/pegawai') + '/' + currentId;
        const res = await fetch(url, { headers: { 'Accept': 'application/json' }, cache: 'no-store' });
        const json = await res.json();
        handleCsrfFromJson(json);
        if (!res.ok || !json.success) throw new Error(json.message || 'Gagal memuat data');
    ...
    document.querySelectorAll('.btn-delete').forEach(btn => btn.onclick = async (e) => {
      ...
      if (window.Swal) {
        const result = await Swal.fire({
          icon:'warning',
          title:'Arsipkan data?',
          html:`Pegawai <b>${escapeHtml(name)}</b> akan diarsip (soft delete).`,
      ...
      if (window.CSRF?.name && window.CSRF?.hash) {
        let tokenInput = f.querySelector(`input[name="${window.CSRF.name}"]`);
        if (!tokenInput) {
    ...
    document.querySelectorAll('.btn-restore').forEach(btn => btn.onclick = async () => {
      const id = btn.dataset.id;
      if (!id) return;
      const url = (window.APP?.pegawai || '/admin/pegawai') + '/' + id + '/restore';

      const fd = new FormData();
      if (window.CSRF?.name && window.CSRF?.hash) fd.set(window.CSRF.name, window.CSRF.hash);

      try {
        const res = await fetch(url, { method: 'POST', body: fd, headers: { 'Accept': 'application/json' }, cache: 'no-store' });
        const json = await res.json();
        handleCsrfFromJson(json);
        if (!res.ok || !json.success) throw new Error(json.message || 'Gagal memulihkan data');
        if (window.Swal) Swal.fire({icon:'success',title:'Dipulihkan',timer:1200,showConfirmButton:false});
        fetchList();
      } catch (e) {
        if (window.Swal) Swal.fire({icon:'error',title:'Gagal',text:e.message||'Terjadi kesalahan'}); else alert(e.message||'Terjadi kesalahan');
      }
  ...
  filterSel && filterSel.addEventListener('change', () => { state.filter = filterSel.value || 'all'; state.page = 1; fetchList(); });

  btnAdd && btnAdd.addEventListener('click', () => {
    currentId = null;
    titleEl && (titleEl.textContent = 'Tambah Pegawai');
    form?.reset();
    const m = form?.querySelector('#_method'); if (m) m.value = 'POST';
    clearErrors();
    modal?.show();
    setTimeout(() => {
      document.querySelector('#empModal .modal-body')?.scrollTo({ top: 0 });
  ...
  btnExport && btnExport.addEventListener('click', () => {
    const u = new URL(window.APP?.pegawaiExport || '/admin/pegawai/export', window.location.origin);
    u.searchParams.set('q', state.q || '');
    window.location.href = u.toString();
  });

  togglePwd?.addEventListener('click', () => { const i = form?.password; if (!i) return; i.type = i.type === 'password' ? 'text' : 'password'; });
  ...
  form?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      modal?.hide();
      if (window.Swal) Swal.fire({ icon:'success', title: isUpdate ? 'Data diperbarui' : 'Data ditambahkan', timer: 1200, showConfirmButton: false, timerProgressBar: true });
      fetchList();
    } catch (err) {

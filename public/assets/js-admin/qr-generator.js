(() => {
  ...
  const compressDataURL = (dataUrl, maxW = 1200, maxH = 1200, q = 0.85) =>
    new Promise((res) => {
      const img = new Image();
      img.onload = () => {
        ...
        const type = dataUrl.startsWith("data:image/png")
          ? "image/png"
          : "image/jpeg";
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    btnGanti?.addEventListener("click", (e) => {
      e.preventDefault();
      fileInput?.click();
    });
    ...
    fileInput?.addEventListener("change", async (e) => {
      ...
      const dataUrl = await compressDataURL(raw, 1600, 1600, 0.9);
      applyPhoto(dataUrl);
    });
    if (dz) {
    ...
    function classForStatus(code) {
      switch (code) {
        case "NORMAL":
          return "badge text-bg-success status-badge";
        case "RUSAK_RINGAN":
          return "badge text-bg-warning status-badge";
        case "RUSAK_BERAT":
          return "badge text-bg-danger status-badge";
        default:
          return "badge text-bg-secondary status-badge";
      }
    }
    ...
    form?.addEventListener("submit", async (e) => {
      ...
      let status = (fd.get("status") || "NORMAL")
        .toString()
        .trim()
        .toUpperCase()
        .replace(/\s+/g, "_");
    ...
    document.getElementById("btnPrint")?.addEventListener("click", () => {
      try {
        if (!sessionStorage.getItem("lastQR")) {
          swalErr("Gagal", "Buat QR dulu.");
          return;
        }
      } catch {}
    ...
    document.getElementById("btnJson")?.addEventListener("click", () => {
      ...
      try {
        state =
          sessionStorage.getItem("lastQR") &&
          JSON.parse(sessionStorage.getItem("lastQR"));
      } catch {}
      if (!state) {
        swalErr("Gagal simpan", "Buat QR dulu.");
        return;
      }
      const payload = {
        token: state.token,
